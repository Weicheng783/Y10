<template>
    <div>
        ...
        <p class="title is-3" id="white-text" style="padding-bottom: 25px;">Meet new cool people here!</p>

        <div v-for="person in persons" :key="person.id" class="FollowingBlock">
            ...
            <div style="text-align:right; padding: 0px 20px 20px 0px;">
                ...
                <button class="button followButton" @click="followUser(person.username)">Follow</button>
            </div>
        </div>
    </div>
</template>
  ...
  name: 'Recommended',
    ...
    methods: {
        ...
        followUser(username) {
            var objectToken = localStorage.getItem('jwt');
            var currentToken = JSON.parse( objectToken );
            axios.post('http://localhost:8000/following/',{username: username}, {
                headers: {
                    Authorization: "Bearer " + currentToken.access,
                    'Accept' : 'application/json',
                    'Content-Type': 'application/json',
                },
            }).then(() => {this.updateRecommended();});
        },
        updateRecommended() {
            var objectToken = localStorage.getItem('jwt');
            var currentToken = JSON.parse( objectToken );
            axios.get('http://localhost:8000/recommended/', {
                headers: {
                    Authorization: "Bearer " + currentToken.access,
                },
            }).then(response => {
                ...
                for(let i=0;i<response.data.length;i++) {
                    ...
                    let currentPoster = response.data[i]['additionaluser__profile_picture'];
